    ...
    **Functions**
    ...
    def initial(self):
        ...
        if checkOption('inflow'):
            print('\n-----------Start of inflow initial')
            # localGauges = returnBool('InLocal')# annotated by Yuancheng Xu, 2024/05/17

            # inflowPointsMap is a CWatM compressed 1D map, in which pixels that have inflow
            # is represented by 1 and others are 0
            where = "InflowPoints"
            ...
            if len(coord) % 2 == 0:
                ...
                if os.path.exists(inflowPointsMap):
                    ...
                    inflowPoints = loadmap(where).astype(np.int64)
                else:
                    if len(coord) == 1:
            ...
            # print('\n------inflowPoints.shape = ', inflowPoints.shape, np.where(inflowPoints > 0), inflowPoints[4054])
            ...
            print('\n sampleInflow = ', self.var.sampleInflow)
            ...
            for name in inflowFile:
                ...
                print('tempTssData = ', tempTssData.shape, tempTssData)
                if flagFirstTss:
