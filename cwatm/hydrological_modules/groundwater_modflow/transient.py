    ...
    **Functions 2**
    ...
    def dynamic(self):
        ...
        if self.var.GW_pumping:
            ...
            # print('\n---------------- abstraction nan check: ', groundwater_abstraction.shape, np.isnan(groundwater_abstraction).any(), np.isnan(groundwater_abstraction).all() )  
            # np.save('./abstraction_check', groundwater_abstraction)
            # print('\n---------------- modfPumpingM nan check: ', self.var.modfPumpingM.shape, np.isnan(self.var.modfPumpingM).any(), np.isnan(self.var.modfPumpingM).all() )  
            # np.save('./modfPumpingM', self.var.modfPumpingM)
            ...
            # print('\n---------------- gwAvail_weights nan check: ', self.gwAvail_weights.shape, np.isnan(self.gwAvail_weights).any(), np.isnan(self.gwAvail_weights).all() )  
            # np.save('./gwAvail_weights', self.gwAvail_weights)
            groundwater_abstraction2 = groundwater_abstraction2 * self.gwAvail_weights
            # np.save('F:/gwAvail_weights', self.gwAvail_weights)
