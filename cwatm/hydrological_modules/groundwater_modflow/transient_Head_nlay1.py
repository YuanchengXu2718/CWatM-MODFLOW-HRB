    ...
    **Functions 2**
    ...
    def initial(self):
        ...
        if self.var.modflow:
            ...
            # by Yuancheng Xu, 21/01/2025
            if 'permeability_coef' in binding:
                self.permeability_coef = load_aquifer_coeff(self, var = 'permeability_coef', nlay = nlay,  mult =  1)
                # print('\n permeability_coef = True ', self.permeability_coef)
                self.permeability = self.permeability * self.permeability_coef
                self.permeability_v = self.permeability_v * self.permeability_coef
            
            # load porosity map/parameter  
            self.porosity = load_aquifer_coeff(self, var = 'poro', nlay = nlay)
            
            # by Yuancheng Xu, 21/01/2025
            if 'poro_coef' in binding:
                self.poro_coef = load_aquifer_coeff(self, var = 'poro_coef', nlay = nlay,  mult =  1)
                # print('\n poro_coef = True ', self.poro_coef)
                self.porosity = self.porosity * self.poro_coef
            ...
            if self.var.load_init_water_table:
                ...
                # modified by Yuancheng, 2025/02/23
                head = self.layer_boundaries[0:nlay] + soildepth_modflow - start_watertabledepth
                head = np.where(head > self.layer_boundaries[1:(nlay+1)], head, self.layer_boundaries[1:(nlay+1)])
            ...
            self.s_stor = np.zeros((nlay, self.domain['nrow'], self.domain['ncol']), dtype=np.float32)
            ...
            ## END BUILDING WELLS_MASK
                ...
                if 'water_table_limit_for_pumping' in binding:
                    ...
                    self.var.availableGWStorageFraction = loadmap('water_table_limit_for_pumping')
                if verboseGW:
                    print('=> Pumping in the ModFlow layer is prevented if water table is under ', self.var.availableGWStorageFraction, ' of the layer capacity')
            ...
            self.var.flow_x = globals.inZero.copy()
            self.var.flow_y = globals.inZero.copy()
            self.var.flow_z = globals.inZero.copy()
            ...
            for lyr in range(nlay):
                ...
                head_min_ref = satFrac_min * (self.layer_boundaries[lyr] - self.layer_boundaries[lyr + 1])

                self.gwavailable_n_lyrs[lyr] =  (self.layer_boundaries[lyr] - self.layer_boundaries[lyr + 1]) * \
                    ( (self.s_stor[lyr] * np.maximum((head[lyr]-self.layer_boundaries[lyr + 1]) * satFrac - \
                        head_min_ref * satFrac_min ,0)) + \
                        (self.s_yield[lyr] * np.maximum(satFrac - satFrac_min, 0)) * (self.confinedAquifer_flags[lyr] > 0) )
    ...
    def dynamic(self):
        ...
        for No in range(6):
            vars(self.var)["gwRch_" + str(No)] = self.var.fracVegCover[No] * vars(self.var)["gwRecharge"][No]
        ...
        self.var.depth0 = self.var.depth[0]
        self.var.flow_x = compressArray(self.modflow2CWATM(self.modflow.flow_x[0]))
        self.var.flow_y = compressArray(self.modflow2CWATM(self.modflow.flow_y[0]))
        self.var.flow_z = compressArray(self.modflow2CWATM(self.modflow.flow_z[0]))
