# By Yuancheng Xu, 2024/05/09
# add MODFLOW first water head to CWatM outputs

# import libraries.decompress
import numpy as np
import os
    ...
    **Functions 2**
    ...
    def initial(self):
        ...
        if self.var.modflow:
            ...
            self.nlay = nlay   #Yuancheng Xu, 2024/05/09
            ...
            self.topography = topography #Yuancheng Xu, 2024/05/09
            ...
            min_thickness = 50
            max_thickness = 200
            thickness[0][np.isnan(thickness[0])] = min_thickness
            thickness[0][thickness[0] < min_thickness] = min_thickness
            thickness[0][thickness[0] > max_thickness] = max_thickness

            thickness[1][np.isnan(thickness[1])] = min_thickness
            thickness[1][thickness[1] < min_thickness] = min_thickness
            thickness[1][thickness[1] > max_thickness] = max_thickness

            print('>> Modflow aquifer thiskness: ')
            print('mean thickness[0])', np.mean(thickness[0]))
            print('mean thickness[1])', np.mean(thickness[1]))
            ...
            # np.save('./modflow_porosity.npy', self.porosity)
            ...
            # print('\n--------------------in transient.py confinedAquifer_flags shape: ', self.confinedAquifer_flags.shape)
            # np.save('./confinedAquifer_flags.npy', self.confinedAquifer_flags)
            ...
            # print('\n------------------------------   CWatM_index shape', self.indices['CWatM_index'].shape)
            # print('\n------------------------------   area shape', self.indices['area'].shape)
            # print('\n------------------------------ data type check ', self.indices['CWatM_index'].dtype, self.indices['area'].dtype)
            
            indices_cell_area = np.bincount(self.indices['CWatM_index'], weights=self.indices['area'],
                                            minlength=maskinfo['mapC'][0])
                                            # minlength=518400)
            #------------------------------- Yuancheng check 24/01/11
            # np.save('./0Test_cwatm_index', self.indices['CWatM_index'])            
            # np.save('./0Test_modflow_index', self.indices['ModFlow_index'])  
            # print('\n------------------------------   minlength', maskinfo['mapC'][0])
            # print('\n------------------------------   self.indices shape', self.indices['CWatM_index'].shape)
            # print('\n------------------------------   maskinfo[shape] = ', maskinfo['shape'][0], maskinfo['shape'][1])
            # print('\n------------------------------   transient.py area_correction')
            # print('\n------------------------------ shape of cellArea: ', self.var.cellArea.shape)
            # print('\n------------------------------ shape of decompress cellArea: ', decompress(self.var.cellArea, nanvalue=0).shape)
            # print('\n------------------------------ shape of indices_cell_area: ', indices_cell_area.shape)
            # np.save('./0Test_indices_cell_area', indices_cell_area)            
            # np.save('./0Test_area_correction_raw', decompress(self.var.cellArea, nanvalue=0) / indices_cell_area)            
            area_correction = (decompress(self.var.cellArea, nanvalue=0) / indices_cell_area)[self.indices['CWatM_index']]
            # np.save('./0Test_area_correction', area_correction)           
            # exit()
            # SUGGESTED BY LUCA
            ...
            if self.var.load_init_water_table:
                # print('------load_init_water_table = True')
                if verboseGW:
                ...
                # print('--start_watertabledepthIntFlag = ', start_watertabledepthIntFlag)
            ...
            if self.var.GW_pumping:
                ...
                # np.save('F:/modflow_basin', self.modflow_basin)
                # np.save('F:/wells_mask', self.wells_mask)
            ...
            ## END BUILDING WELLS_MASK
                ...
                print('----final modflow-initialize')
            ...
            self.var.depth = self.var.head*0 # initialize
            for lyr in range(nlay):
                self.var.depth[lyr,:] = compressArray(self.modflow2CWATM(topography - head[lyr]))
            self.var.depth0 = self.var.depth[0]
            self.var.depth1 = self.var.depth[1]
    ...
    def dynamic(self):
        ...
        print('\n--------------basin.shape = ', self.modflow.basin.shape)
        ...
        # print('\n---------------- recharge nan check: ', np.isnan(groundwater_recharge_modflow).any(), np.isnan(groundwater_recharge_modflow).all() )  
        # np.save('./recharge_check', groundwater_recharge_modflow)
        ...
        if self.var.GW_pumping:
            ...
            groundwater_abstraction2 = np.array([groundwater_abstraction * wellsMask] * nlay_dyn)   #copy to 2-dimensions
            
            self.gwAvail_weights = np.minimum(divideArrays(self.groundwater_storage_available, \
                                                           self.CWATM2modflow(decompress(self.var.groundwater_storage_available))), 1.0)
            ...
            # np.save('F:/StorageInAbstractionCells', StorageInAbstractionCells)
            ...
            # np.save('F:/proportional_groundwater_storage', proportional_groundwater_storage)
            # np.save('F:/groundwater_abstract', groundwater_abstraction)
            ...
            # give the information to ModFlow
            # print('\n---------------- abstraction2 nan check: ', np.isnan(groundwater_abstraction2).any(), np.isnan(groundwater_abstraction2).all() )  
            # np.save('./abstraction2_check2', groundwater_abstraction2)
            # groundwater_abstraction2 = np.random.uniform(0, 1, (2, 171, 141))
            ...
            # np.save('F:/groundwater_abstract2', groundwater_abstraction2)
            
        # print('\n---------------- head check 1 isnan.all(): ', np.isnan(self.modflow.head).any(), np.isnan(self.modflow.head).all() )  
        # np.save('./head_check_res', self.modflow.head)
        # np.save('./head_check_res_de', self.modflow.decompress(self.modflow.head.astype(np.float32)))
        ...
        for lyr in range(self.nlay):
            self.var.head[lyr,:] = compressArray(self.modflow2CWATM(head[lyr]))
        # np.save('E:/CWatM24/Saq-NCP/output_test/head0.npy', self.var.head[0])

        for lyr in range(self.nlay):
            self.var.depth[lyr,:] = compressArray(self.modflow2CWATM(self.topography - head[lyr]))
        self.var.depth0 = self.var.depth[0]
        self.var.depth1 = self.var.depth[1]
        # np.save('E:/CWatM24/Saq-NCP/output_test/depth0.npy', self.var.depth[0])
        # print('\n---------------- head check 2 isnan.all(): ', np.isnan(self.modflow.head).any(), np.isnan(self.modflow.head).all() )  
        # np.save('./head_check_res2', self.modflow.head)
        # head = np.random.uniform(100, 300, (2, 1213, 1218))
        # MODIF LUCA
        if self.var.writeerror:
        ...
        for lyr in range(nlay_dyn): 
            satFrac = self.calcSaturatedCellFraction(lyr = lyr, head = head)
            # print('--satFrac shape: ', satFrac.shape)
            # np.save('F:/satFrac', satFrac)
            self.groundwater_storage_n_layer[lyr] = (self.layer_boundaries[lyr] - self.layer_boundaries[lyr + 1]) * satFrac * (self.s_stor[lyr] * head[lyr] + self.s_yield[lyr] * (self.confinedAquifer_flags[lyr] > 0))
