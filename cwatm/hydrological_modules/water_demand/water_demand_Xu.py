...
from cwatm.hydrological_modules.water_demand.irrigation_Xu import waterdemand_irrigation
...
# Yuancheng Xu, 2024/10/26
from cwatm.hydrological_modules.water_demand.diversion_yellow_Xu import diversion_yellow
# Yuancheng Xu, 2025/02/02
from cwatm.hydrological_modules.water_demand.diversion_SNWDC_Xu import diversion_SNWDC
# PB1507
...
class water_demand:
    ...
    **Functions**
    ...
    def __init__(self, model):
        ...
        # Yuancheng Xu, 2024/10/26
        self.diversion_yellow = diversion_yellow(model)
        # Yuancheng Xu, 2025/02/02
        self.diversion_SNWDC = diversion_SNWDC(model)  
    ...
    def initial(self):
        ...
        if "includeWastewater" in binding:
            self.var.includeWastewater = returnBool('includeWastewater')
        ...
        if checkOption('includeWaterDemand'):
            ...
            if 'sectorSourceAbstractionFractions' in option:
                if checkOption('sectorSourceAbstractionFractions'):
                    ...
                    if self.var.includeWastewater:
                        self.var.wwtAbstractionFraction_Domestic = loadmap(
                            'wwtAbstractionFraction_Domestic')
                        self.var.wwtAbstractionFraction_Livestock = loadmap(
                            'wwtAbstractionFraction_Livestock')
                        self.var.wwtAbstractionFraction_Industry = loadmap(
                            'wwtAbstractionFraction_Industry')
                        self.var.wwtAbstractionFraction_Irrigation = loadmap(
                            'wwtAbstractionFraction_Irrigation')
                        
                        recycleFrac_path = cbinding('recycleFrac')
                        recycleFrac_temp = []
                        with open(recycleFrac_path, 'r') as file:
                            for line in file:
                                recycleFrac_temp.append([float(line.split()[0]), float(line.split()[1])])
                        self.var.recycleFrac_year = np.array(recycleFrac_temp)
            ...
            if not self.var.sectorSourceAbstractionFractions:# add by Yuancheng Xu, 21/01/2025
                # -------------------------------------------
            ...
            # by Yuancheng Xu, 2025/05/29
            self.var.ReplenM3 = globals.inZero.copy()
            self.var.ReplenM3_SNWDC = globals.inZero.copy()
            self.var.ReplenM3_wwt = globals.inZero.copy()
            self.var.act_wwt = globals.inZero.copy()
            ...
            self.var.totalIrrDemand_dyn = globals.inZero.copy()
        ...
        # Yuancheng Xu, 2024/10/26
        if "includeDiversionYellow" in binding:
            self.var.includeDiversionYellow = returnBool('includeDiversionYellow')
        else:
            self.var.includeDiversionYellow = False
        
        if self.var.includeDiversionYellow:
            self.diversion_yellow.initial()

        # Yuancheng Xu, 2025/02/02
        if "includeDiversionSNWDC" in binding:
            self.var.includeDiversionSNWDC = returnBool('includeDiversionSNWDC')
        else:
            self.var.includeDiversionSNWDC = False

        # Yuancheng Xu, 2025/02/16
        if "includeReplenSNWDC" in binding:
            self.var.includeReplenSNWDC = returnBool('includeReplenSNWDC')
        else:
            self.var.includeReplenSNWDC = False

        if "CoefReplen" in binding:
            self.var.CoefReplen = loadmap("CoefReplen")
        else:
            self.var.CoefReplen = 0.8
        
        if self.var.includeDiversionSNWDC:
            self.diversion_SNWDC.initial()

        # by Yuancheng Xu, 2025/07/13
        # current model only have one unconfined aquifers
        # if includeConfinedAbs = True, 
        # pumping from confined aquifers was excluded from the abstraction in modflow  
        self.var.includeConfinedAbs = False
        if "includeConfinedAbs" in binding:
            self.var.includeConfinedAbs = returnBool('includeConfinedAbs')
        if self.var.includeConfinedAbs:
            self.var.confinedAbs = globals.inZero.copy()
            confinedFrac_path = cbinding('confinedFrac')
            confinedFrac_temp = []
            with open(confinedFrac_path, 'r') as file:
                for line in file:
                    confinedFrac_temp.append([float(line.split()[0]), float(line.split()[1])])
            self.var.confinedFrac_year = np.array(confinedFrac_temp)             
    ...
    def dynamic(self):
        ...
        * get non-Irrigation water demand and its return flow fraction
        ...
        # by Yuancheng Xu, 2025/05/30
        self.var.ReplenM3 = globals.inZero.copy()

        if self.var.modflow:

            if self.var.includeIndusDomesDemand:  # all demands are taken into account
                self.var.envFlow = 0.00001
        ...
        if checkOption('includeWaterDemand'):   # True in CWatM-NCP
            ...
            if self.var.waterdemandFixed: # False in CWatM-NCP
            ...
            if self.var.includeIndusDomesDemand:  # all demands are taken into account
                ...
                self.irrigation.dynamic(wd_date)
                self.environmental_need.dynamic()
            else:
                self.irrigation.dynamic(wd_date)
            ...
            # Yuancheng Xu, 2024/10/26
            if self.var.includeDiversionYellow:
                self.diversion_yellow.dynamic(wd_date)

            # Yuancheng Xu, 2025/02/02
            if self.var.includeDiversionSNWDC:
                self.diversion_SNWDC.dynamic(wd_date)

            # Yuancheng Xu, 2024/10/28
            # if the input water demand file is monthly, self.var.domesticDemand is updated monthly
            # update the demand after interbasin water diversion
            # so that it is not necessary to update every following expression
            self.var.domesticDemand_dyn = self.var.domesticDemand.copy()
            if self.var.livestockDemand == 0:
                self.var.livestockDemand_dyn = self.var.livestockDemand
            else:
                self.var.livestockDemand_dyn = self.var.livestockDemand.copy()
            self.var.industryDemand_dyn = self.var.industryDemand.copy()
            self.var.totalIrrDemand_dyn = self.var.totalIrrDemand.copy()
            ...
            if self.var.includeIndusDomesDemand:  # all demands are taken into account
                ...
                self.var.domesticDemand_dyn = globals.inZero.copy()
                self.var.industryDemand_dyn = globals.inZero.copy()
                self.var.livestockDemand_dyn = globals.inZero.copy()
                self.var.nonIrrDemand = globals.inZero.copy()
            ...
            # Yuancheng Xu, 2024/10/26
            # water diversion from the Yellow River
            # Yuancheng Xu, 2025/02/02
            # water diversion from SNWDC
            if self.var.sectorSourceAbstractionFractions:
                if self.var.includeDiversionSNWDC:
                    self.var.DivSNWDC_Domestic = globals.inZero.copy()
                    self.var.DivSNWDC_Livestock = globals.inZero.copy()
                    self.var.DivSNWDC_Industry = globals.inZero.copy()
                    self.var.DivSNWDC_Irrigation = globals.inZero.copy()
                    self.var.ReplenM3_SNWDC = globals.inZero.copy()
                    self.var.residualDivSNWDC = 0

                    for subID in self.var.subdivideDataSNWDC.columns.values:
                        maskSNWDC = self.var.maskDivSNWDC == subID
                        subAvlDivSNWDC = self.var.AvlDivSNWDC * self.var.subdivideSNWDC[subID]

                        pot_DivSNWDC_Domestic = self.var.AbstractionFraction_DivSNWDC_Domestic * self.var.domesticDemand_dyn * maskSNWDC
                        pot_DivSNWDC_Livestock = self.var.AbstractionFraction_DivSNWDC_Livestock * self.var.livestockDemand_dyn * maskSNWDC
                        pot_DivSNWDC_Industry = self.var.AbstractionFraction_DivSNWDC_Industry * self.var.industryDemand_dyn * maskSNWDC
                        pot_DivSNWDC_Irrigation = self.var.AbstractionFraction_DivSNWDC_Irrigation * self.var.totalIrrDemand_dyn * maskSNWDC                
                        
                        pot_DivSNWDC_Domestic_KM3 = np.nansum(pot_DivSNWDC_Domestic * self.var.cellArea / 1e9)
                        pot_DivSNWDC_Livestock_KM3 = np.nansum(pot_DivSNWDC_Livestock * self.var.cellArea / 1e9)
                        pot_DivSNWDC_Industry_KM3 = np.nansum(pot_DivSNWDC_Industry * self.var.cellArea / 1e9)
                        pot_DivSNWDC_Irrigation_KM3 = np.nansum(pot_DivSNWDC_Irrigation * self.var.cellArea / 1e9)
                                            
                        self.var.DivSNWDC_Domestic += pot_DivSNWDC_Domestic * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Domestic_KM3)
                        subAvlDivSNWDC = subAvlDivSNWDC - pot_DivSNWDC_Domestic_KM3 * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Domestic_KM3)
                        
                        if pot_DivSNWDC_Livestock_KM3 > 0:
                            self.var.DivSNWDC_Livestock += pot_DivSNWDC_Livestock * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Livestock_KM3)
                            subAvlDivSNWDC = subAvlDivSNWDC - pot_DivSNWDC_Livestock_KM3 * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Livestock_KM3)                    
                        else:
                            self.var.DivSNWDC_Livestock += 0
                        
                        self.var.DivSNWDC_Industry += pot_DivSNWDC_Industry * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Industry_KM3)
                        subAvlDivSNWDC = subAvlDivSNWDC - pot_DivSNWDC_Industry_KM3 * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Industry_KM3)                    
                        
                        if pot_DivSNWDC_Irrigation_KM3 > 0:
                            self.var.DivSNWDC_Irrigation += pot_DivSNWDC_Irrigation * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Irrigation_KM3)
                            self.var.residualDivSNWDC += subAvlDivSNWDC - pot_DivSNWDC_Irrigation_KM3 * np.minimum(1.0, subAvlDivSNWDC/pot_DivSNWDC_Irrigation_KM3)
                        else:
                            self.var.DivSNWDC_Irrigation += 0
                            self.var.residualDivSNWDC += subAvlDivSNWDC

                    # by Yuancheng Xu, 2025/02/16
                    # river replenishment using SNWDC water
                    self.var.ReplenM3_SNWDC = globals.inZero.copy()
                    if self.var.includeReplenSNWDC:
                        if wd_date.month <= 6 or wd_date.month >= 9:
                            self.var.DivSNWDC_Replen = self.var.CoefReplen * self.var.residualDivSNWDC
                            # km3/d to m3/d
                            self.var.ReplenM3_SNWDC_tot = self.var.DivSNWDC_Replen * 1e9
                            self.var.residualDivSNWDC = (1. - self.var.CoefReplen) * self.var.residualDivSNWDC
                            # print('\n----------residualDivSNWDC = ', self.var.residualDivSNWDC)

                            for key in self.var.sampleReplen:
                                loc = self.var.sampleReplen[key]
                                self.var.ReplenM3_SNWDC[loc] = self.var.ReplenM3_SNWDC_tot / self.var.noReplenPoints
                ...
                self.var.ReplenM3 = self.var.ReplenM3 + self.var.ReplenM3_SNWDC
                self.var.act_DivSNWDC = self.var.DivSNWDC_Domestic + self.var.DivSNWDC_Livestock + self.var.DivSNWDC_Industry + self.var.DivSNWDC_Irrigation

                # update the demand after interbasin water diversion
                # tend not to update every following code
                self.var.domesticDemand_dyn = self.var.domesticDemand_dyn - self.var.DivSNWDC_Domestic
                self.var.livestockDemand_dyn = self.var.livestockDemand_dyn - self.var.DivSNWDC_Livestock
                self.var.industryDemand_dyn = self.var.industryDemand_dyn - self.var.DivSNWDC_Industry
                self.var.totalIrrDemand_dyn = self.var.totalIrrDemand_dyn - self.var.DivSNWDC_Irrigation

                if self.var.includeDiversionYellow:
                    self.var.DivYel_Domestic = globals.inZero.copy()
                    self.var.DivYel_Livestock = globals.inZero.copy()
                    self.var.DivYel_Industry = globals.inZero.copy()
                    self.var.DivYel_Irrigation = globals.inZero.copy()
                    self.var.residualDivYel = 0                    

                    for subID in self.var.subdivideDataYel.columns.values:
                        maskYel = self.var.maskDivYellow == subID
                        subAvlDivYel = self.var.AvlDivYel * self.var.subdivideYel[subID]
                        # print("\n Yel ratio: ", subID, self.var.AvlDivYel, self.var.subdivideYel[subID])
                        # print("\n Yel sumMask: ", subID, np.nansum(maskYel))

                        pot_DivYellow_Domestic = self.var.AbstractionFraction_DivYellow_Domestic * self.var.domesticDemand_dyn * maskYel
                        pot_DivYellow_Livestock = self.var.AbstractionFraction_DivYellow_Livestock * self.var.livestockDemand_dyn * maskYel
                        pot_DivYellow_Industry = self.var.AbstractionFraction_DivYellow_Industry * self.var.industryDemand_dyn * maskYel
                        pot_DivYellow_Irrigation = self.var.AbstractionFraction_DivYellow_Irrigation * self.var.totalIrrDemand_dyn * maskYel               

                        pot_DivYellow_Domestic_KM3 = np.nansum(pot_DivYellow_Domestic * self.var.cellArea / 1e9)
                        pot_DivYellow_Livestock_KM3 = np.nansum(pot_DivYellow_Livestock * self.var.cellArea / 1e9)
                        pot_DivYellow_Industry_KM3 = np.nansum(pot_DivYellow_Industry * self.var.cellArea / 1e9)
                        pot_DivYellow_Irrigation_KM3 = np.nansum(pot_DivYellow_Irrigation * self.var.cellArea / 1e9)
                    
                        self.var.DivYel_Domestic += pot_DivYellow_Domestic * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Domestic_KM3)
                        subAvlDivYel = subAvlDivYel - pot_DivYellow_Domestic_KM3 * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Domestic_KM3)
                        if pot_DivYellow_Livestock_KM3 > 0:
                            self.var.DivYel_Livestock += pot_DivYellow_Livestock * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Livestock_KM3)
                            subAvlDivYel = subAvlDivYel - pot_DivYellow_Livestock_KM3 * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Livestock_KM3)                    
                        else:
                            self.var.DivYel_Livestock += 0
                        self.var.DivYel_Industry += pot_DivYellow_Industry * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Industry_KM3)
                        subAvlDivYel = subAvlDivYel - pot_DivYellow_Industry_KM3 * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Industry_KM3)                    
                        self.var.DivYel_Irrigation += pot_DivYellow_Irrigation * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Irrigation_KM3)
                        self.var.residualDivYel += subAvlDivYel - pot_DivYellow_Irrigation_KM3 * np.minimum(1.0, subAvlDivYel/pot_DivYellow_Irrigation_KM3)
                        # print("\n Yel residual: ", subID, self.var.residualDivYel)
                else:
                    self.var.DivYel_Domestic = globals.inZero.copy()
                    self.var.DivYel_Livestock = globals.inZero.copy()
                    self.var.DivYel_Industry = globals.inZero.copy()
                ...
                self.var.act_DivYel = self.var.DivYel_Domestic + self.var.DivYel_Livestock + self.var.DivYel_Industry + self.var.DivYel_Irrigation

                # update the demand after interbasin water diversion
                # tend not to update every following code
                self.var.domesticDemand_dyn = self.var.domesticDemand_dyn - self.var.DivYel_Domestic
                self.var.livestockDemand_dyn = self.var.livestockDemand_dyn - self.var.DivYel_Livestock
                self.var.industryDemand_dyn = self.var.industryDemand_dyn - self.var.DivYel_Industry
                self.var.totalIrrDemand_dyn = self.var.totalIrrDemand_dyn - self.var.DivYel_Irrigation
            ...
            if self.var.sectorSourceAbstractionFractions:
                if self.var.includeDesal:
                    pot_Desal_Domestic = self.var.othAbstractionFraction_Desal_Domestic * self.var.domesticDemand_dyn
                    pot_Desal_Livestock = self.var.othAbstractionFraction_Desal_Livestock * self.var.livestockDemand_dyn
                    pot_Desal_Industry = self.var.othAbstractionFraction_Desal_Industry * self.var.industryDemand_dyn
                    pot_Desal_Irrigation = self.var.othAbstractionFraction_Desal_Irrigation * self.var.totalIrrDemand_dyn
            ...
            if self.var.sectorSourceAbstractionFractions:
                if self.var.includeDesal:
                    self.var.Desal_Domestic = np.minimum(self.var.act_DesalWaterAbstractM,
                                                        self.var.othAbstractionFraction_Desal_Domestic * self.var.domesticDemand_dyn)
                    self.var.Desal_Livestock = np.minimum(self.var.act_DesalWaterAbstractM - self.var.Desal_Domestic,
                                                            self.var.othAbstractionFraction_Desal_Livestock * self.var.livestockDemand_dyn)
                    self.var.Desal_Industry = np.minimum(
                        self.var.act_DesalWaterAbstractM - self.var.Desal_Domestic - self.var.Desal_Livestock,
                        self.var.othAbstractionFraction_Desal_Industry * self.var.industryDemand_dyn)
                    self.var.Desal_Irrigation = np.minimum(
                        ...
                        self.var.othAbstractionFraction_Desal_Irrigation * self.var.totalIrrDemand_dyn)
                else:
            ...
            if self.var.sectorSourceAbstractionFractions:
                ...
                pot_Channel_Domestic = self.var.swAbstractionFraction_Channel_Domestic * (self.var.domesticDemand_dyn - self.var.Desal_Domestic)
                pot_Channel_Livestock = self.var.swAbstractionFraction_Channel_Livestock * (self.var.livestockDemand_dyn - self.var.Desal_Livestock)
                pot_Channel_Industry = self.var.swAbstractionFraction_Channel_Industry * (self.var.industryDemand_dyn - self.var.Desal_Industry)
                pot_Channel_Irrigation = self.var.swAbstractionFraction_Channel_Irrigation * (self.var.totalIrrDemand_dyn - self.var.Desal_Irrigation)
            ...
            if checkOption('includeWaterBodies'):
                ...
                self.var.AvlWwt = globals.inZero.copy()
                # First apply to wastewater reclamation, e.g., restricted use reservoirs. Follow by lakes and reservoirs (not restricted)                
                if self.var.includeWastewater:
                    self.var.recycleFrac = self.var.recycleFrac_year[np.where(self.var.recycleFrac_year[:, 0] == \
                                                                              dateVar['currDate'].year), 1][0][0]
                    # print('\n recycleFrac = ', self.var.recycleFrac)

                    if dateVar['newStart']:
                        self.var.act_indWithdrawal_pre = globals.inZero.copy()
                        self.var.act_domWithdrawal_pre = globals.inZero.copy()
                    AvlWwt = self.var.recycleFrac * (self.var.act_indWithdrawal_pre + self.var.act_domWithdrawal_pre)
                    self.var.AvlWwt = AvlWwt

                    if self.var.sectorSourceAbstractionFractions:
                        pot_wwt_Domestic = np.minimum(
                            self.var.wwtAbstractionFraction_Domestic * self.var.domesticDemand_dyn,
                            self.var.domesticDemand_dyn - self.var.Desal_Domestic - self.var.Channel_Domestic)
                
                        pot_wwt_Livestock = np.minimum(
                            self.var.wwtAbstractionFraction_Livestock * self.var.livestockDemand_dyn,
                            self.var.livestockDemand_dyn - self.var.Desal_Livestock - self.var.Channel_Livestock)

                        pot_wwt_Industry = np.minimum(
                            self.var.wwtAbstractionFraction_Industry * self.var.industryDemand_dyn,
                            self.var.industryDemand_dyn - self.var.Desal_Industry - self.var.Channel_Industry)

                        pot_wwt_Irrigation = np.minimum(
                            self.var.wwtAbstractionFraction_Irrigation * self.var.totalIrrDemand_dyn,
                            self.var.totalIrrDemand_dyn - self.var.Desal_Irrigation - self.var.Channel_Irrigation)

                    self.var.wwt_Domestic = np.where(pot_wwt_Domestic < AvlWwt, pot_wwt_Domestic, AvlWwt)
                    AvlWwt = AvlWwt - self.var.wwt_Domestic
                    self.var.wwt_Livestock = np.where(pot_wwt_Livestock < AvlWwt, pot_wwt_Livestock, AvlWwt)
                    AvlWwt = AvlWwt - self.var.wwt_Livestock
                    self.var.wwt_Industry = np.where(pot_wwt_Industry < AvlWwt, pot_wwt_Industry, AvlWwt)
                    AvlWwt = AvlWwt - self.var.wwt_Industry
                    self.var.wwt_Irrigation = np.where(pot_wwt_Irrigation < AvlWwt, pot_wwt_Irrigation, AvlWwt)
                    AvlWwt = AvlWwt - self.var.wwt_Irrigation

                    self.var.act_wwt = self.var.wwt_Domestic + self.var.wwt_Livestock + self.var.wwt_Industry + self.var.wwt_Irrigation
                    self.var.ReplenM3_wwt = AvlWwt * self.var.cellArea
                    self.var.ReplenM3 = self.var.ReplenM3 + self.var.ReplenM3_wwt
                else:
                    self.var.wwt_Domestic = globals.inZero.copy()
                    self.var.wwt_Industry = globals.inZero.copy()
                    self.var.wwt_Livestock = globals.inZero.copy()
                ...
                if self.var.sectorSourceAbstractionFractions:
                    ...
                    pot_Lake_Domestic = np.minimum(
                        self.var.swAbstractionFraction_Lake_Domestic * self.var.domesticDemand_dyn,
                        self.var.domesticDemand_dyn - self.var.Desal_Domestic - self.var.Channel_Domestic - self.var.wwt_Domestic)
                    pot_Lake_Livestock = np.minimum(
                        self.var.swAbstractionFraction_Lake_Livestock * self.var.livestockDemand_dyn,
                        self.var.livestockDemand_dyn - self.var.Desal_Livestock - self.var.Channel_Livestock - self.var.wwt_Livestock)

                    pot_Lake_Industry = np.minimum(
                        self.var.swAbstractionFraction_Lake_Industry * self.var.industryDemand_dyn,
                        self.var.industryDemand_dyn - self.var.Desal_Industry - self.var.Channel_Industry - self.var.wwt_Industry)

                    pot_Lake_Irrigation = np.minimum(
                        self.var.swAbstractionFraction_Lake_Irrigation * self.var.totalIrrDemand_dyn,
                        self.var.totalIrrDemand_dyn - self.var.Desal_Irrigation - self.var.Channel_Irrigation - self.var.wwt_Irrigation)
                ...
                if self.var.sectorSourceAbstractionFractions:
                    ...
                    pot_Res_Domestic = np.minimum(
                        self.var.swAbstractionFraction_Res_Domestic * self.var.domesticDemand_dyn,
                        self.var.domesticDemand_dyn - self.var.Desal_Domestic - self.var.Channel_Domestic - self.var.wwt_Domestic - self.var.Lake_Domestic)

                    pot_Res_Livestock = np.minimum(
                        self.var.swAbstractionFraction_Res_Livestock * self.var.livestockDemand_dyn,
                        self.var.livestockDemand_dyn - self.var.Desal_Livestock - self.var.Channel_Livestock - self.var.wwt_Livestock - self.var.Lake_Livestock)

                    pot_Res_Industry = np.minimum(
                        self.var.swAbstractionFraction_Res_Industry * self.var.industryDemand_dyn,
                        self.var.industryDemand_dyn - self.var.Desal_Industry - self.var.Channel_Industry - self.var.wwt_Industry - self.var.Lake_Industry)

                    pot_Res_Irrigation = np.minimum(
                        self.var.swAbstractionFraction_Res_Irrigation * self.var.totalIrrDemand_dyn,
                        self.var.totalIrrDemand_dyn - self.var.Desal_Irrigation - self.var.Channel_Irrigation - self.var.wwt_Irrigation - self.var.Lake_Irrigation)
            ...
            if self.var.sectorSourceAbstractionFractions:
                pot_GW_Domestic = np.minimum(
                    self.var.gwAbstractionFraction_Domestic * self.var.domesticDemand_dyn,
                    self.var.domesticDemand_dyn - self.var.Desal_Domestic - self.var.Channel_Domestic - self.var.wwt_Domestic - self.var.Lake_Domestic - self.var.Res_Domestic)

                pot_GW_Livestock = np.minimum(
                    self.var.gwAbstractionFraction_Livestock * self.var.livestockDemand_dyn,
                    self.var.livestockDemand_dyn - self.var.Desal_Livestock - self.var.Channel_Livestock - self.var.wwt_Livestock - self.var.Lake_Livestock - self.var.Res_Livestock)

                pot_GW_Industry = np.minimum(
                    self.var.gwAbstractionFraction_Industry * self.var.industryDemand_dyn,
                    self.var.industryDemand_dyn - self.var.Desal_Industry - self.var.Channel_Industry - self.var.wwt_Industry - self.var.Lake_Industry - self.var.Res_Industry)

                pot_GW_Irrigation = np.minimum(
                    self.var.gwAbstractionFraction_Irrigation * self.var.totalIrrDemand_dyn,
                    self.var.totalIrrDemand_dyn - self.var.Desal_Irrigation - self.var.Channel_Irrigation - self.var.wwt_Irrigation - self.var.Lake_Irrigation - self.var.Res_Irrigation)

                self.var.pot_GroundwaterAbstract = pot_GW_Domestic + pot_GW_Livestock + pot_GW_Industry + pot_GW_Irrigation
                # np.save('./pot_GWAbs.npy', self.var.pot_GroundwaterAbstract)
                if np.isnan(self.var.pot_GroundwaterAbstract).any():
                    print('\n----------------------- WARNING! NaN exists in self.var.pot_GroundwaterAbstract (water_demand.py)  ----Yuancheng 24/01/20')
                    self.var.pot_GroundwaterAbstract[np.isnan(self.var.pot_GroundwaterAbstract)] = 0
            else:
            ...
            if self.var.sectorSourceAbstractionFractions:
                ...
                unmet_Domestic = self.var.domesticDemand_dyn - self.var.Desal_Domestic - self.var.Channel_Domestic - self.var.wwt_Domestic - self.var.Lake_Domestic - self.var.Res_Domestic - self.var.GW_Domestic
                unmet_Livestock = self.var.livestockDemand_dyn - self.var.Desal_Livestock - self.var.Channel_Livestock - self.var.wwt_Livestock - self.var.Lake_Livestock - self.var.Res_Livestock - self.var.GW_Livestock
                unmet_Industry = self.var.industryDemand_dyn - self.var.Desal_Industry - self.var.Channel_Industry - self.var.wwt_Industry - self.var.Lake_Industry - self.var.Res_Industry - self.var.GW_Industry
                unmet_Irrigation = self.var.totalIrrDemand_dyn - self.var.Desal_Irrigation - self.var.Channel_Irrigation - self.var.wwt_Irrigation - self.var.Lake_Irrigation - self.var.Res_Irrigation - self.var.GW_Irrigation
            ...
            if checkOption('limitAbstraction'):
                ...
                if self.var.sectorSourceAbstractionFractions:
                    ...
                    # modified by Yuancheng Xu, 2024/10/29
                    self.var.Suf_Domestic = self.var.Channel_Domestic + self.var.Lake_Domestic + self.var.Res_Domestic
                    self.var.Suf_Industry = self.var.Channel_Industry + self.var.Lake_Industry + self.var.Res_Industry
                    self.var.Suf_Irrigation = self.var.Channel_Irrigation + self.var.Lake_Irrigation + self.var.Res_Irrigation
                    self.var.act_nonIrrWithdrawal = self.var.Desal_Domestic + self.var.Desal_Livestock + self.var.Desal_Industry + \
                                                    ...
                                                    self.var.DivYel_Domestic + self.var.DivYel_Livestock + self.var.DivYel_Industry + \
                                                    self.var.DivSNWDC_Domestic + self.var.DivSNWDC_Livestock + self.var.DivSNWDC_Industry
                    self.var.act_irrWithdrawal = self.var.Desal_Irrigation + self.var.Channel_Irrigation + self.var.wwt_Irrigation + \
                                                 self.var.Lake_Irrigation + self.var.Res_Irrigation + self.var.GW_Irrigation + self.var.DivYel_Irrigation + self.var.DivSNWDC_Irrigation
                    ...
                    act_irrWithdrawalGW = self.var.totalIrrDemand_dyn * (1 - act_swAbstractionFraction)
                    act_irrWithdrawalGW = np.minimum(self.var.nonFossilGroundwaterAbs - act_nonIrrWithdrawalGW,
                                                     act_irrWithdrawalGW)
                    act_irrWithdrawalSW = act_swAbstractionFraction * self.var.totalIrrDemand_dyn
                ...
                if self.var.includeIndusDomesDemand:  # all demands are taken into account
                    self.var.unmetDemand = (self.var.totalIrrDemand_dyn - self.var.act_irrWithdrawal) + \
                ...
                if self.var.modflow:
                    ...
                    self.var.act_irrWithdrawal = np.copy(self.var.totalIrrDemand_dyn)
            ...
            if self.var.modflow:
                if self.var.GW_pumping:  # pumping demand is sent to ModFlow (used in transient module)
                    ...
                    self.var.modfPumpingM = globals.inZero.copy()
                    if self.var.includeConfinedAbs:
                        self.var.confinedFrac = self.var.confinedFrac_year[np.where(self.var.confinedFrac_year[:, 0] == \
                                                                              dateVar['currDate'].year), 1][0][0]
                        self.var.confinedPumping = globals.inZero.copy()
                        self.var.confinedPumping += act_gw * self.var.confinedFrac
                        self.var.modfPumpingM += act_gw * (1. - self.var.confinedFrac)
                    else:
            ...
            if self.var.sectorSourceAbstractionFractions:
                ...
                self.var.act_indWithdrawal = self.var.Desal_Industry + self.var.Channel_Industry + self.var.wwt_Industry + self.var.Lake_Industry + \
                                             self.var.Res_Industry + self.var.GW_Industry + self.var.DivYel_Industry + self.var.DivSNWDC_Industry
                self.var.act_domWithdrawal = self.var.Desal_Domestic + self.var.Channel_Domestic + self.var.wwt_Domestic + self.var.Lake_Domestic + \
                                             self.var.Res_Domestic + self.var.GW_Domestic + self.var.DivYel_Domestic + self.var.DivSNWDC_Domestic
                self.var.act_indWithdrawal_pre = self.var.act_indWithdrawal.copy()
                self.var.act_domWithdrawal_pre = self.var.act_domWithdrawal.copy()
                self.var.act_livWithdrawal = self.var.Desal_Livestock + self.var.Channel_Livestock + self.var.wwt_Livestock + self.var.Lake_Livestock + \
                                             self.var.Res_Livestock + self.var.GW_Livestock + self.var.DivYel_Livestock + self.var.DivSNWDC_Livestock
            ...
            # by Yuancheng Xu, 2025/07/21
            self.var.addtoevapotrans = self.var.addtoevapotrans + self.var.act_totalIrrConsumption

            if self.var.sectorSourceAbstractionFractions:
                # by Yuancheng Xu, 2025/07/22
                self.var.returnflowNonIrr = self.var.act_nonIrrWithdrawal - self.var.act_nonIrrConsumption - self.var.AvlWwt
            ...
            # print("\ncheck rfNonIrr = ", np.nansum(self.var.returnflowNonIrr))
            # print("\ncheck rfIrr = ", np.nansum(self.var.returnflowIrr))
            ...
            if checkOption('calcWaterBalance'):
                ...
                self.model.waterbalance_module.waterBalanceCheck(
                    [self.var.nonIrrDemand, self.var.totalIrrDemand_dyn],  # In
