...
# -------------------------------------------------------------------------
# Edited by Yuancheng Xu 2024/10/15
# read agricultural water use from netcdf file
#--------------------------------------------------------------------------
...
class waterdemand_irrigation:
    ...
    **Functions**
    ...
    def initial(self):
        ...
        # by Yuancheng Xu, 2024/10/16
        if "irrTimeMonthly" in binding:
            if returnBool('irrTimeMonthly'):
                self.var.irrTime = 'monthly'
            else:
                self.var.irrTime = 'yearly'
        else:
            self.var.irrTime = 'monthly'

        if "irrWithdrawalvarname" in binding:
            self.var.irrWithdrawalVar = cbinding("irrWithdrawalvarname")
        else:
            self.var.irrWithdrawalVar = "agrWithdrawal"
    ...
    def dynamic(self, wd_date):
        ...
        * get non-Irrigation water demand and its return flow fraction
        """
        if "useAgrDemand" in binding:
            useAgrDemand = returnBool('useAgrDemand')
        else:
            useAgrDemand = False
        # print('\ncalcIrr = ', calcIrr)
        ...
        if useAgrDemand:
            # print("\ncalcIrr = True")
            """
            Dynamic part of the water demand module - agricultural use 
            modified from the domestic version
            read monthly (or yearly) water demand from netcdf and transform (if necessary) to [m/day]

            """

            # temporal resolution of netcdf is the same as domestic
            if self.var.irrTime == 'monthly':
                new = 'newMonth'
            else:
                new = 'newYear'

            if globals.dateVar['newStart'] or globals.dateVar[new]:
                self.var.irrNonpaddyDemand = readnetcdf2('irrWaterDemandFile', wd_date, self.var.irrTime,
                                                    value=self.var.irrWithdrawalVar)
                # avoid small values (less than 1 m3):
                self.var.irrNonpaddyDemand = np.where(self.var.irrNonpaddyDemand > self.var.InvCellArea,
                                                self.var.irrNonpaddyDemand, 0.0)
                ...
                if not self.var.demand_unit:
                    ...
                    self.var.irrNonpaddyDemand = self.var.irrNonpaddyDemand * 1000000 * self.var.M3toM / timediv
                
                self.var.irrDemand[3] = divideValues(self.var.irrNonpaddyDemand, self.var.fracVegCover[3])
                self.var.irrDemand[2] = self.var.irrDemand[3] * 0 # no paddy in the NCP
                self.var.irrPaddyDemand = self.var.irrNonpaddyDemand * 0 # no paddy in the NCP
