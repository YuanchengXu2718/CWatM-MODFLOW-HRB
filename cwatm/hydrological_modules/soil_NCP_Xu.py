...
# -------------------------------------------------------------------------
# KUnsat updated by Yuancheng Xu based on Yang & Long (WRR, 2022)
# Updated in Feb. 2024
# -------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
    ...
    def dynamic(self, coverType, No):
        ...
        # load soil_alpha from initial file ------Yuancheng Xu
        soil_alpha=loadmap('soil_alpha')

        # False in Saq-NCP, Yuancheng Xu, 2025/02/13
        ...
        if self.var.modflow:
            ...
            kUnSat2 = self.var.KSat2[NoSoil] * np.exp(-1.*soil_alpha*((self.var.thetas2[NoSoil]-self.var.theta2[NoSoil])/self.var.thetas2[NoSoil]))
            kUnSat3 = self.var.KSat3[NoSoil] * np.exp(-1.*soil_alpha*((self.var.thetas3[NoSoil]-self.var.theta3[NoSoil])/self.var.thetas3[NoSoil]))
            ...
            # added by Yuancheng Xu, 2024/05/20
            # make sure w2 & w3 >= 0
            capRise1 = np.maximum(0, np.minimum(capRise1, self.var.w2[No] - self.var.wres2[No]))
            capRise2 = np.maximum(0, np.minimum(capRise2, self.var.w3[No] - self.var.wres3[No]))
            # ------------------------------------------------------------------
        ...
        if np.any(self.var.w1[No] < 0):
            print('\nw1 < 0  --check 0')
        if np.any(self.var.w2[No] < 0):
            print('\nw2 < 0  --check 0')
        ...
        self.var.w1[No] = self.var.w1[No] - ta1     # annotated by Yuancheng Xu, 24/03/26
        self.var.w2[No] = self.var.w2[No] - ta2
        self.var.w3[No] = self.var.w3[No] - ta3   # annotated by Yuancheng Xu, 24/03/29

        if np.any(self.var.w1[No] < 0):
            print('\nw1 < 0  --check 1')
        if np.any(self.var.w2[No] < 0):
            print('\nw2 < 0  --check 1')
        if np.any(np.isnan(self.var.w1[No])):
            print('\nNan exits in w1[', No, ']  --check 1')
        ...
        self.var.w1[No] = self.var.w1[No] - self.var.actBareSoilEvap[No]   # annotated by Yuancheng Xu, 24/03/26
        if np.any(np.isnan(self.var.w1[No])):
            print('\nNan exits in w1[', No, ']  --check 2')
        ...
        if np.any(relSat < 0):
            print('\nWARNING: relSat < 0, Yuancheng Xu, 2024/05/20')
            print('\n w1 < 0: ', np.any(self.var.w1[No] < 0), 'w2 < 0: ', np.any(self.var.w2[No] < 0))
            relSat = np.maximum(relSat, 0)
        ...
        if coverType == 'irrPaddy' or not(checkOption('preferentialFlow')):
            ...
            if np.any(np.isnan(self.var.prefFlow[No])):
                print('\nNan check for availWaterInfiltration, relSat, relSat ** self.var.cPrefFlow',\
                    np.any(np.isnan(availWaterInfiltration)),  np.any(np.isnan(relSat)),\
                        np.any(np.isnan(relSat ** self.var.cPrefFlow)))
                self.var.prefFlow[No][np.isnan(self.var.prefFlow[No])] = 0
                np.save('./relSat.npy', relSat)
                np.save('./cPref.npy', self.var.cPrefFlow)
        ...
        if np.any(np.isnan(self.var.infiltration[No])):
            print('\nNan exits in infiltration[', No, ']  --check 3')
            print('\nNan check for potInf, availWaterInfiltration, self.var.prefFlow[No], self.var.FrostIndex',\
                  np.any(np.isnan(potInf)), np.any(np.isnan(availWaterInfiltration)), \
                    np.any(np.isnan(self.var.prefFlow[No])), self.var.FrostIndex)
            self.var.infiltration[No][np.isnan(self.var.infiltration[No])] = 0
        ...
        self.var.w1[No] = self.var.w1[No] + self.var.infiltration[No]    # annotated by Yuancheng Xu, 24/03/26
        self.var.w2[No] = self.var.w2[No] + np.where(self.var.w1[No] > self.var.ws1[No], self.var.w1[No] - self.var.ws1[No], 0)
        self.var.w1[No] = np.minimum(self.var.ws1[No], self.var.w1[No])   # annotated by Yuancheng Xu, 24/03/26
        if np.any(self.var.w1[No] < 0):
            print('\nw1 < 0  --check 3')
        if np.any(self.var.w2[No] < 0):
            print('\nw2 < 0  --check 3')
        if np.any(np.isnan(self.var.w1[No])):
            print('\nNan exits in w1[', No, ']  --check 3')
        ...
        self.var.w1[No] = self.var.w1[No] + capRise1    # annotated by Yuancheng Xu, 24/03/26
        self.var.w2[No] = self.var.w2[No] - capRise1 + capRise2
        if self.var.modflow:
            self.var.w3[No] = self.var.w3[No] - capRise2
            # GW capillary rise has already been added to the soil
        else:
            self.var.w3[No] = self.var.w3[No] - capRise2 + self.var.capRiseFromGW[No]  # annotated by Yuancheng Xu, 24/03/29
        if np.any(self.var.w1[No] < 0):
            print('\nw1 < 0  --check 4')
        if np.any(self.var.w2[No] < 0):
            print('\nw2 < 0  --check 4')
        if np.any(np.isnan(self.var.w1[No])):
            print('\nNan exits in w1[', No, ']  --check 4')
        # Percolation -----------------------------------------------
        # Available water in both soil layers [m]
        availWater1 = np.maximum(0.,self.var.w1[No] - self.var.wres1[No])
        availWater2 = np.maximum(0.,self.var.w2[No] - self.var.wres2[No])
        ...
        # kUnSat2 = self.var.KSat2[NoSoil] * np.sqrt(satTerm2) * np.square(1 - (1 - satTerm2 ** self.var.genuInvM2[NoSoil]) ** self.var.genuM2[NoSoil])
        # kUnSat3 = self.var.KSat3[NoSoil] * np.sqrt(satTerm3) * np.square(1 - (1 - satTerm3 ** self.var.genuInvM3[NoSoil]) ** self.var.genuM3[NoSoil])
        kUnSat1 = self.var.KSat1[NoSoil] * np.exp(-1.*soil_alpha*((self.var.thetas1[NoSoil]-self.var.theta1[NoSoil])/self.var.thetas1[NoSoil]))
        kUnSat2 = self.var.KSat2[NoSoil] * np.exp(-1.*soil_alpha*((self.var.thetas2[NoSoil]-self.var.theta2[NoSoil])/self.var.thetas2[NoSoil]))
        kUnSat3 = self.var.KSat3[NoSoil] * np.exp(-1.*soil_alpha*((self.var.thetas3[NoSoil]-self.var.theta3[NoSoil])/self.var.thetas3[NoSoil]))
        ...
        self.var.w1[No] = self.var.w1[No] - self.var.perc1to2[No]    # annotated by Yuancheng Xu, 24/03/26
        self.var.w2[No] = self.var.w2[No] + self.var.perc1to2[No] - self.var.perc2to3[No]
        self.var.w3[No] = self.var.w3[No] + self.var.perc2to3[No] - self.var.perc3toGW[No]   # annotated by Yuancheng Xu, 24/03/29
        if np.any(self.var.w1[No] < 0):
            print('\nw1 < 0  --check 5')
        if np.any(self.var.w2[No] < 0):
            print('\nw2 < 0  --check 5')
        if np.any(np.isnan(self.var.w1[No])):
            print('\nNan exits in w1[', No, ']  --check 5')
        ...
        if self.var.modflow:
            ...
            lc_rechargeShare = divideValues(self.var.gwRecharge[No]*self.var.fracVegCover[No], self.var.sum_gwRecharge)
