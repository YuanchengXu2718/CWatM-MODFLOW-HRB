...
from cwatm.hydrological_modules.lakes_reservoirs_Xu import *
...
# --------------------------------------------------------------------------
    ...
    def initial(self):
        ...
        * calculate manning's roughness coefficient
        ...
        # edited by Yuancheng Xu, make sure there is no 0 in self.var.chanWettedPerimeterAlpha 2024/03/17
        self.var.chanWettedPerimeterAlpha = np.maximum(0.1, self.var.chanWidth + 2 * 0.5 * self.var.chanDepth)
...
# ---------------------------------------------------------------------------------
        ...
        # by Yuancheng, 2025/02/16
        # river replenishment using SNWD-C water
        if self.var.includeReplenSNWDC:
            self.var.QDeltaR = (self.var.ReplenM3 - self.var.QReM3Old) / self.var.noRoutingSteps
        ...
        for subrouting in range(self.var.noRoutingSteps):
            ...
            # river replenishment using SNWD-C water
            if self.var.includeReplenSNWDC:
                self.var.ReplenDt = (self.var.QReM3Old + (subrouting + 1) * self.var.QDeltaR) / self.var.noRoutingSteps
                sideflowChanM3 += self.var.ReplenDt
            ...
            # by Yuancheng Xu, 2025/02/19
            # check routing
            self.var.runoffM3 = runoffM3
            self.var.lakesResOut = lakesResOut
            self.var.sideflowChanM3 = sideflowChanM3
            self.var.sum_landSurfaceRunoffM3 = self.var.sum_landSurfaceRunoff * self.var.cellArea / self.var.noRoutingSteps
            self.var.baseflowM3 = self.var.baseflow * self.var.cellArea / self.var.noRoutingSteps
            self.var.leakageIntoRunoffM3 = self.var.leakageIntoRunoff * self.var.cellArea / self.var.noRoutingSteps
        ...
        # by Yuancheng, 2025/02/16
            # river replenishment using SNWD-C water
        if self.var.includeReplenSNWDC:
            self.var.QReM3Old = self.var.ReplenM3.copy()
