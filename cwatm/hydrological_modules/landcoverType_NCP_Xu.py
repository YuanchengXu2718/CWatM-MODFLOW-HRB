    ...
    **Functions**
    ...
    def initial(self):
        ...
        * Water covered area No.5
        ...
        """
        self.var.rejected_recharge_previous_day = globals.inZero.copy()
        ...
        for coverType in self.var.coverTypes[:4]:
            ...
            # kUnSat3FC = self.var.KSat3[j] * np.sqrt(satTerm3FC) * np.square(1 - (1 - satTerm3FC ** self.var.genuInvM3[j]) ** self.var.genuM3[j])
            
            # load soil_alpha from initial file ------Yuancheng Xu
            soil_alpha=loadmap('soil_alpha')
            kUnSat1FC = self.var.KSat1[j] * np.exp(-1.*soil_alpha*((self.var.thetas1[j]-self.var.theta1[j])/self.var.thetas1[j]))
            kUnSat2FC = self.var.KSat2[j] * np.exp(-1.*soil_alpha*((self.var.thetas2[j]-self.var.theta2[j])/self.var.thetas2[j]))
            kUnSat3FC = self.var.KSat3[j] * np.exp(-1.*soil_alpha*((self.var.thetas3[j]-self.var.theta3[j])/self.var.thetas3[j]))
            self.var.kunSatFC12.append(np.sqrt(kUnSat1FC * kUnSat2FC))
            self.var.kunSatFC23.append(np.sqrt(kUnSat2FC * kUnSat3FC))
        ...
        for coverType in self.var.coverTypes[:4]:
            ...
            self.var.w1[i] = self.var.load_initial(coverType + "_w1",self.var.wwp1[i])
            self.var.w2[i] = self.var.load_initial(coverType + "_w2",self.var.wwp2[i])
            self.var.w3[i] = self.var.load_initial(coverType + "_w3",self.var.wwp3[i])
            ...
            # annotated by Yuancheng Xu, 2024/03/28
            # seems this part should not be in the loop
            """
            soilVars = ['w1', 'w2', 'w3']
        ...
        # move this part out of the loop, Yuancheng Xu, 2024/03/28
        #-----------------------------------------------------------------------------------------------------------
        soilVars = ['w1', 'w2', 'w3']
        for variable in soilVars:
            vars(self.var)["sum_" + variable] = globals.inZero.copy()
        ...
        * calculate the fraction of 6 land cover types based on the maps
        ...
        if init and dynamic:
            ...
            if self.var.staticLandCoverMaps: 
                ...
                if returnBool('sealedCali'):
                    sealedCoef = loadmap('sealedCoef')
                    self.var.fracVegCover[1] = 0.5 * (1 - sealedCoef) * self.var.fracVegCover[4] + self.var.fracVegCover[1]
                    self.var.fracVegCover[0] = 0.5 * (1 - sealedCoef) * self.var.fracVegCover[4] + self.var.fracVegCover[0]
                    self.var.fracVegCover[4] = sealedCoef * self.var.fracVegCover[4]
    ...
    def dynamic(self):
        ...
        * calls sealed_water_module.dynamic
        ...
        if self.var.modflow:
            ...
            # modified by Yuancheng Xu, 2025/02/17
            # rivers in the NCP almost dried up in the past, leading to an almost 0 self.var.fracVegCover[5]
            # after river replenishment initiative, rivers have been revived
            # however, the water fraction in landcover dataset has not been updated
            self.var.channelFraction = np.minimum(1.0, self.var.chanWidth * self.var.chanLength / self.var.cellArea)
            self.var.riverbedExchangeM = np.minimum(self.var.leakageriver_factor * np.maximum(self.var.channelFraction, 0) *
                                                    ((1 - self.var.capriseindex + 0.25) // 1),
                                                    0.80 * self.var.readAvlChannelStorageM)  # leakage in m/d
        ...
        for variable in soilVars:
                vars(self.var)["sum_" + variable] = globals.inZero.copy()
                for No in range(4):
                    # print('\n-----in landcoverType dynamic No = ', No)
                    if np.any(np.isnan(vars(self.var)[variable][No])):
                        print('\nNan exits in ', variable, No)
                        # np.save('./' + variable + str(No) +'.npy', vars(self.var)[variable][No])
                    vars(self.var)["sum_" + variable] += self.var.fracVegCover[No] * vars(self.var)[variable][No]



        self.var.sum_topwater = self.var.fracVegCover[2] * self.var.topwater
        #print('irrigated area is ', np.sum(self.var.fracVegCover[3]*self.var.cellArea)/(1000*1000))
        ...
        self.var.sum_runoff = self.var.sum_directRunoff + self.var.sum_interflow + self.var.leakageIntoRunoff + self.var.rejected_recharge_previous_day
