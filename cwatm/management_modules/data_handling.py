...
def maskfrompoint(mask2D, xleft, yup):
    ...
    maskinfo['maskflat'] = mask.ravel()  # map to 1D not compresses
    maskinfo['shapeflat'] = maskinfo['maskflat'].shape  # length of the 1D array
    print('--------------------in data_handling maskfrompoint, mapC.shape = ', mapC.shape)
    maskinfo['mapC'] = mapC.shape  # length of the compressed 1D array
    print('--------------------in data_handling maskfrompoint, maskinfo["mapC"][0] = ', maskinfo['mapC'][0])
    ...
    for maps in meteomaps:
        ...
        for filename in nameall:
            ...
            if startfile == 0:  # search first file where dynamic run starts
                ...
                if (datestartint >= startint) and (datestartint <= endint ): # modified by Yuancheng Xu, 2024/04/02
                    startfile += 1
                    indstart = startint - datestartint
                    indend = dateendint - datestartint
                    meteolist[startfile - 1] = [filename, indstart,indend, start, dateend,]
...
#             if useDaily == "monthly":
    ...
    # if namebinding == 'fractionLandcover':
    # print('\n------------------in readnetcdf2: namebinding = ', namebinding )
    
    if cut:
        cutmap[0], cutmap[1], cutmap[2], cutmap[3] = mapattrNetCDF(name)   #modified by Yuancheng Xu, 2024/03/28
        # print('\n------------------in readnetcdf2: cutmap = ', cutmap )
        if turn_latitude:
